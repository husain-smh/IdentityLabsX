...
function buildLlmPrompt(batch: QuoteRecord[]): string {
  ...
  return `
You are a strict filter. Input is JSON array of quote tweets. Identify only the tweets that explicitly or strongly convey the narrative that this thing is better/faster than Lovable or Bolt for building apps (e.g., "anything is better than Lovable/Bolt", "build apps so fast, way faster than Lovable/Bolt", "better than Lovable/Bolt"). Do not infer if it's not clearly stated. If unclear, exclude it.
